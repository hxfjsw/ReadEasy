...
export const useWordExtraction = (selectedBook: BookshelfItem | null) => {
  ...
  // 批量加载剩余释义（仅针对本地数据库和 AI 没有的单词）
  const batchLoadRemainingDefinitions = useCallback(async (words: ExtractedWord[]) => {
    // 找出还没有释义的单词
    const missingWords = words.filter(w => !w.definitionCn);
    ...
    try {
      ...
      console.error('批量加载剩余释义失败:', error);
    } finally {
  ...
  const openExtractWordsModal = useCallback(async (book: BookshelfItem) => {
    ...
    try {
      // 调用后端提取服务（完整的后端处理：读取文件、提取单词、过滤熟词本/废词本、查询 ECDICT）
      const result = await window.electron.ipcRenderer.invoke('word:extract', book.filePath);
      
      if (result.success && result.data) {
        const { words, masteredCount, ignoredCount } = result.data;
        ...
        setOriginalExtractedWords(sortedWords);
        setExtractedWords(sortedWords);
        
        // 对没有 ECDICT 释义的单词，尝试从本地数据库和 AI 获取
        const missingWords = sortedWords.filter(w => !w.definitionCn);
        if (missingWords.length > 0) {
          setTimeout(() => batchLoadRemainingDefinitions(sortedWords.slice(0, 50)), 100);
        }
      } else {
        message.error(result.message || '提取单词失败');
      }
    } catch (error) {
  ...
  }, [extractSortOrder, batchLoadRemainingDefinitions]);
    ...
    try {
      ...
      if (ecdictResult?.success) {
        ...
        if (result.success) {
          ...
          setExcludedIgnoredCount(prev => prev + invalidWords.length);
  ...
  return {
    ...
    selectedWords,
    setSelectedWords,
    excludedCount: excludedMasteredCount + excludedIgnoredCount,
    ...
    batchLoadDefinitions: batchLoadRemainingDefinitions,
